...
# to execute unit tests and build fdk dotnet dist package on the bitbucket github branch created from release tag branch.
...
(
  # Build dotnet containing src files toß generate a gem file
  docker build -t fdk_dotnet_build_image -f ./internal/docker-files/Dockerfile_build .
  docker run --rm -v $PWD:/build -w /build --env BUILD_VERSION=${BUILD_VERSION} fdk_dotnet_build_image ./internal/build-scripts/build_dist_pkg.sh
)
